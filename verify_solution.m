function [eta, nu, K, t_star, t_double_star] = verify_solution(f, f_u, f_v, f_uu, f_uv, f_vv, b, R)
  % Compute eta
  eta = compute_eta(f, b);
  ...
  if Delta > 0
    ...
    if (inf(t_star) > 0) && (sup(t_star) < R)
      ...
      disp(['t_double_star = ', num2str(sup(t_double_star))])
    else
